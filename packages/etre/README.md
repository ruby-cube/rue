...
This differs from traditional inheritance in that a role can have many prereqs whereas a class can extend only one other class, and it differs from composition through dependency injection in that prereqs cannot be redundant. With dependency injection, if a class A is composed of class B and C, which both inherit class D, class D’s constructor will run twice in order to instantiate class A. In the Être system, prereqs are collected into a set at the time of creating the reifier function, ensuring non-redudancy. 
